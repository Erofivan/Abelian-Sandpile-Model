...
public:
    queue()
        : capacity_(1), buffer_(new T[capacity_]), head_(0), tail_(0), size_(0) {}
    ...
    void pop() {
        if (empty()) {
            std::cerr << "queue::cannot_pop_empty_queue" << std::endl;
            return;
        }
    ...
    T& front() {
        if (empty()) {
            std::cerr << "queue::cannot_get_front_of_empty_queue" << std::endl;
    ...
    T& back() {
        if (empty()) {
            std::cerr << "queue::cannot_get_back_of_empty_queue" << std::endl;
    ...
    public:
        iterator(queue<T>* q, size_t index) : q(q), index(index) {}
        ...
        T& operator*() {
            return q->buffer_[(q->head_ + index) % q->capacity_];
        }
